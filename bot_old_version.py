...
TELEGRAM_BOT_TOKEN = "8565038374:AAEZBx1lWC4JTOUc86ZuEXhVtBoBsS6mxKA"
bot = telebot.TeleBot(TELEGRAM_BOT_TOKEN)  # обращение к библиотеке по токену нашего бота
API_KEY = '760e06df0bd02347f473b8efb010b2ac'  # API WeatherMap
...
#сделай в боте кнопки???
# сделай так, чтобы бот при старте запрашивал у тебя город для прогноза погоды. Именно по этому введённому городу бот будет отправлять каждый день в 5:00 данные
# сделать так, чтобы бот отправлял каждый день прогноз  погоды
# Также нужно сделать функцию смены города.
#работа кнопок
# сохрани возможность просто так узнавать прогноз погоды в городах мира
# надо выгрузить код на бесплатный серсер, чтобы он работал круглосуточно
